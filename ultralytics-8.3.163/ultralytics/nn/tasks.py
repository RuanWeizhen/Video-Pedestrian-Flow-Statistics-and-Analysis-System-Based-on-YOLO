...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            if m is CBAM:
                # CBAM 输出通道数与输入相同，不需要缩放
                c2 = ch[f]  # 输出通道 = 输入通道
                # args 原本可能包含 [reduction, kernel_size]，现在将输入通道数作为第一个参数插入
                args = [ch[f], *args]  # args 变为 [channels, reduction, kernel_size]
                print(f"CBAM layer {i}: final args = {args}, n = {n}")
            else:
                c1, c2 = ch[f], args[0]
            ...
            args = [ch[f]]
        # 在 parse_model 函数内部的 for 循环中，找到类似这样的位置（通常在处理各种模块的 elif 后）
        elif m is EMA:
            # 输入通道来自上一层输出 ch[f]
            c1 = ch[f]
            # 将通道数作为第一个参数传给 EMA，args 中可能包含 factor 等其他参数
            args = [c1, *args]
            # 输出通道与输入相同
            c2 = c1
        elif m is Concat:
        ...
        if m is CBAM:
            print(f"CBAM layer {i}: args before build = {args}, n = {n}")
        ...
        if i == 0:
            ch = []
        ch.append(c2)
        print(f"After layer {i}, ch length = {len(ch)}, last c2 = {c2}")
